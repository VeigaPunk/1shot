...
cli_launch() { local p=$2; case $1 in
  ...
  qwen)     LAUNCH=(qwen --yolo ${MAGGA_MODEL:+--model "$MAGGA_MODEL"} --prompt-interactive "$p") ;;
  omp)      LAUNCH=(omp --auto-approve ${MAGGA_MODEL:+--model "$MAGGA_MODEL"} --thinking max "$p") ;;
  cursor)   LAUNCH=(cursor-agent --yolo --trust --approve-mcps --sandbox disabled ${MAGGA_MODEL:+--model "$MAGGA_MODEL"} "$p") ;;
